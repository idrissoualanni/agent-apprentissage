...
def evaluation_memory_node(state: AgentState, db_path=None) -> dict:
    ...
    # --- Évaluation Quiz ---
        ...
        if competency_id is not None:
            ...
            # Enregistrer la tentative de quiz (une seule fois, avec la session active)
            session_id = _resolve_session_id(state, db_path)
...
def _resolve_session_id(state: AgentState = None, db_path=None) -> Optional[int]:
    ...
    Args:
        state: état courant (fournit ``thread_id`` si disponible)
        db_path: chemin vers la base SQLite (fallback : ``config.DB_PATH``)

    Returns:
        ID de la session correspondante, ou ``None`` si introuvable.
        Fallback : session la plus récente si aucun thread_id n'est connu.
    """
    from db import db as db_module
    ...
    # Si le fichier de base n'existe pas, on ne peut rien retourner.
    ...
    with db_module.get_connection(path) as conn:
        if thread_id:
            ...
            if row:
                return row["id"]
        # Fallback : dernière session ouverte
        row = conn.execute(
...
def _format_quiz_for_display(questions: list) -> str:
    """Texte court d'introduction pour le quiz — le rendu détaillé est fait
    côté UI via ``ui.renderers.render_quiz_html`` pour profiter de l'interactivité.
    On évite un long Markdown en doublon du composant HTML.
    """
    n = len(questions)
